...
riscvILA_user::riscvILA_user(int pc_init_val)
    : model(InstrLvlAbs::New("riscv")), // define ila
      ...
      dmem(model.NewMemState("dmem", MEM_WORD_ADDR_LEN, MEM_WORD)),
      imem(model.NewMemState("imem", MEM_WORD_ADDR_LEN, MEM_WORD)),
      count_cycle(model.NewBvState("count_cycle", 64)),
      count_instr(model.NewBvState("count_instr", 64)),
      inst(FetchFromMem(imem, pc(31, 2))),
...
void riscvILA_user::addInstructions() {
  ...
  // //
  {
    ...
    auto lw_val = LoadFromMem(dmem, addr(31, 2));
    auto nxt_pc = pc + bv(4);
    ...
    // //
    {
      ...
      instr.SetUpdate(count_cycle, count_cycle+7);
      instr.SetUpdate(count_instr, count_instr+1);
  ...
  // //
  {
    ...
    //auto old_val = LoadFromMem(dmem, word_addr);
    auto nxt_pc = pc + bv(4);
    ...
    // //
    {
      ...
      // what exactly is the target value ?
      instr.SetUpdate(dmem, StoreToMem(dmem, word_addr, rs2_val));
