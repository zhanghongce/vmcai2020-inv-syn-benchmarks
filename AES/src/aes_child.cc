...
void AES::AddChild(InstrRef& inst) {
  ...
  auto blk_cnt  = child.NewBvState("blk_cnt", 128);
  ...
  { // LOAD byte-by-byte
    ...
    auto xram_read_addr = address + blk_cnt(15,0) + ZExt(byte_cnt, 16);
  ...
  { // store byte-by-byte
    ...
    auto xram_write_addr = address + blk_cnt(15,0) + ZExt(byte_cnt, 16);
    ...
    instr.SetUpdate(blk_cnt,
                    Ite(byte_cnt == 15,
                        Ite(blk_cnt(15,0) + 16 < length, blk_cnt + 16, blk_cnt),
                        blk_cnt));

    instr.SetUpdate(status,
                    Ite(byte_cnt == 15, // the last byte has been written to
                        Ite(blk_cnt(15,0) + 16 < length, // if we reach the length
